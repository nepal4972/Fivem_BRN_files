...
(() => {
    ...
    QBHud.compassToggle = function(data) {
        if (data.show) {
            $(".compass-container").fadeIn();
        } else {
            $(".compass-container").fadeOut();
        }
    }
    ...
    QBHud.Update = function(data) {
        if(data.type == "cash") {
            ...
            $("#cash").html(data.cash);
            if (data.minus) {
                $(".money-cash").append('<p class="moneyupdate minus">-<span id="cash-symbol">$ &nbsp;</span><span><span id="minus-changeamount">' + data.amount + '</span></span></p>')
                ...
                $(".money-cash").append('<p class="moneyupdate plus">+<span id="cash-symbol">$ &nbsp;</span><span><span id="plus-changeamount">' + data.amount + '</span></span></p>')
        ...
        if(data.type == "bank") {
            ...
            $("#bank").html(data.bank);
            if (data.minus) {
                $(".money-bank").append('<p class="moneyupdate minus">-<span id="cash-symbol">$ &nbsp;</span><span><span id="minus-changeamount">' + data.amount + '</span></span></p>')
                $(".minus").css("display", "block");
                setTimeout(function() {
                    $(".minus").fadeOut(750, function() {
                        $(".minus").remove();
                ...
                $(".money-bank").append('<p class="moneyupdate plus">+<span id="cash-symbol">$ &nbsp;</span><span><span id="plus-changeamount">' + data.amount + '</span></span></p>')
                $(".plus").css("display", "block");
                setTimeout(function() {
                    $(".plus").fadeOut(750, function() {
                        $(".plus").remove();
        ...
        window.addEventListener('message', function(event) {
            switch(event.data.action) {
                ...
                case "car":
                    ...
                    break;
                case "compassToggle":
                    QBHud.compassToggle(event.data);
                    break;
                case "seatbelt":
...
window.addEventListener('message', function (event) {
	...
	switch (event.data.action) {
        case 'updateOxygen':
            widthHeightSplit(event.data.bleeding,  $('.ui-bleedbarfill'));
    }
});
